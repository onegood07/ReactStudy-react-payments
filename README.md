...
- [ ] 카드 번호 입력 및 식별
  - [V] 카드 번호의 3~4번 블럭은 숨김 처리한다.
  - [V] 숫자를 입력하지 않으면 올바르게 입력하라는 피드백을 보여주고, 입력을 제한한다.
  - [V] 각 카드 번호 입력 블럭은 0~9의 숫자 4자리로 이루어져있다.
  - [V] 입력은 숫자만 가능하며, 유효하지 않은 번호 입력 시 피드백을 제공한다.
...
- [V] 카드 소유자 이름 입력
  - [V] 소문자로 입력 시 강제로 대문자로 변환한다.
  - [V] 영어가 아닌 문자 입력 시 입력을 제한하고 피드백을 제공한다.
  - [V] (추가) 사용자 이름은 최소 두글자 이상 입력해야 한다.
- [V] 실시간 프리뷰 업데이트
